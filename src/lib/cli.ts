...
const readline = createInterface(process.stdin, process.stdout),
    commands = new Map<string, (args: Map<string, string>) => void>([
        ["clear", console.clear],
        ["exit", process.exit.bind(process, 0)]
    ]);

function parse(input: string): { command: string, args: [string, string][]; }[] {
    let mode: "command" | "argument_name" | "argument_value" | "unknown" = "command";

    const commands: { command: string, args: [string, string][]; }[] = [];
    let currentCommand = {
        command: "",
                ...
                switch (char) {
                    ...
                    case " ": {
                        if (currentCommand.args.at(-1)?.[0].length) {
                            mode = "argument_value";
                            break;
                        } else throw new Error("Parsing error: found empty flag");
                    }
                    default:
                        (currentCommand.args.at(-1) ?? (() => currentCommand.args[0] = ["", ""] as [string, string])())[0] += char;
                }
                break;
            }
            case "argument_value": {
                switch (char) {
                    case "\"": {
                        if (argument_name_stringMode) {
                            mode = "unknown";
                            currentCommand.args.push(["", ""] as [string, string]);
                            argument_name_stringMode = false;
                        } else {
